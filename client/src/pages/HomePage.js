...
const HomePage = () => {
    ...
    useEffect(() => {
        try {
            ...
            if (savedMatch) {
                ...
                console.log("Loaded Guest Match from localStorage:", parsedMatch);
                setGuestMatch(parsedMatch);
            }
        } catch (error) { console.error("Could not parse guest match", error); localStorage.removeItem('guestMatch'); }
    }, []);
    ...
    if (guestMatch && guestMatch.innings) {
        ...
        // --- DEBUGGING CONSOLE LOG ---
        console.log("Rendering Scoreboard with data:", currentInningsData);
        
        return (
            <>
                {guestMatch.result && <Celebration />}
                <div className="scoreboard">
                    <div className="top-right-nav" style={{ position: 'absolute', top: '20px', right: '20px', display: 'flex', gap: '10px' }}> <Link to="/login"><button>Login</button></Link> <Link to="/register"><button>Register</button></Link> </div>
                    ...
                    {guestMatch.status === 'Innings Break' && (<button onClick={() => handleGuestScoreUpdate({runs: 0, isWicket: false, isExtra: false})} style={{padding: '15px', fontSize: '18px', backgroundColor: 'green', color: 'white'}}>Start 2nd Innings</button>)}
                    {guestMatch.status === 'In Progress' && (<div className="controls"><h3>Score a Ball</h3><button onClick={() => handleGuestScoreUpdate(0, false, false)}>0</button><button onClick={() => handleGuestScoreUpdate(1, false, false)}>1</button><button onClick={() => handleGuestScoreUpdate(4, false, false)}>4</button><button onClick={() => handleGuestScoreUpdate(6, false, false)}>6</button><button onClick={() => handleGuestScoreUpdate(0, false, true)} style={{ backgroundColor: '#f0ad4e' }}>No Ball</button><button onClick={() => handleGuestScoreUpdate(0, false, true)} style={{ backgroundColor: '#f0ad4e' }}>Wide</button><button onClick={() => handleGuestScoreUpdate(0, true, false)} style={{ backgroundColor: '#d9534f', color: 'white' }}>Wicket</button></div>)}
